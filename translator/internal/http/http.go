...
import (
	...
	httpClient "github.com/VladimirDronik/touchon-server/http/client"
...
func New(cfg map[string]string, store store.Store, mqttClient *client.Client, wsServer *ws.Server, ringBuffer fmt.Stringer, logger *logrus.Logger) (*Server, error) {
	switch {
	case cfg == nil:
	...
	case mqttClient == nil:
		return nil, errors.Wrap(errors.New("mqttClient is nil"), "http.New")
	case wsServer == nil:
		return nil, errors.Wrap(errors.New("wsServer is nil"), "http.New")
	}

	baseServer, err := server.New("API", cfg, ringBuffer, logger)
	if err != nil {
...
//	if err := o.Server.Start(o.GetConfig()["http_addr"]); err != nil {
...
func (o *Server) createSession(deviceID int) (*model.Tokens, error) {
	...
	if err := o.store.Users().AddRefreshToken(deviceID, tokens.RefreshToken, refreshTokenTTL); err != nil {
		return nil, errors.Wrap(err, "createSession")
	}
