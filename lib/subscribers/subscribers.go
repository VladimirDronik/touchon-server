...
func New(handlersCount int) *Subscribers {
	return &Subscribers{
		...
		currHandlerID: 0,
		handlers:      make(map[int]interfaces.MsgHandler, handlersCount),
...
type Subscribers struct {
	handlersCount int
	currHandlerID int
	handlers      map[int]interfaces.MsgHandler // Все зарегистрированные обработчики
	columns       map[string]map[string]*intset.IntSet
}
...
func (o *Subscribers) GetHandlers(msg interfaces.Message) []interfaces.MsgHandler {
	values := map[string]string{
		"names":        msg.GetName(),
	...
	for _, v := range []string{string(msg.GetType()), ""} {
		if s2, ok := o.columns["types"][v]; ok {
			s.UnionWith(s2)
		}
	}
	...
	r := make([]interfaces.MsgHandler, 0, 10)
