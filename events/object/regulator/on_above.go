...
func init() {
	maker := func() (*event.Event, error) {
		e := &event.Event{
			...
			Name:        "Текущее значение больше заданного",
			Description: "",
...
func NewOnAboveMessage(topic string, targetID int, value float32) (messages.Message, error) {
	e, err := event.MakeEvent("object.regulator.on_above", messages.TargetTypeObject, targetID, map[string]interface{}{"value": value})
	if err != nil {
		return nil, errors.Wrap(err, "NewOnAboveMessage")
	}
